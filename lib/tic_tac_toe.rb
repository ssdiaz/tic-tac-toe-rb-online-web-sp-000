...
def won?(board) # return winning combo index, or false if no win combo
 WIN_COMBINATIONS.find do |winning_combo| # find first  winning_combo (array of 3: [0,1,2]) in WIN_COMBINATIONS
    ...
    if position_taken?(board, winning_combo[0]) &&
        board[winning_combo[0]] == board[winning_combo[1]] &&
          board[winning_combo[1]] == board[winning_combo[2]]
          winning_combo # return the win_combination indexes that won
    end
  end
end
